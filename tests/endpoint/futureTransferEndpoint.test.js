...
describe('FutureTransferEndpoint', () => {
    it('should response with success', async () => {
        ...
        const responseTransfers = await client.transfer().future().list();
    ...
    it('should response with fail for unauthenticated client', async () => {
        ...
        await expect(client.transfer().future().list()).rejects.toThrow(HttpException);
    });
});
